      ...
      <el-form ref="form" :label-position="'top'" :model="form" label-width="80px">
        <div class="info">
          <el-form-item label="产品备案号" prop="recordnum">
            <el-input v-model="form.recordnum" :disabled="list.isrecord == 2||disabled?true:false" @blur="validatePass">
            </el-input>
          </el-form-item>
          ...
          <el-form-item label="类别" prop="arrcate" :rules="[{ required: true, message: '请输入产品类别', trigger: 'blur' }]">
            <el-select v-model="form.arrcate" multiple placeholder="请选择">
              <el-option v-for="item in procatelist" :key="item.id" :label="item.name" :value="item.id"></el-option>
            </el-select>
          </el-form-item>
        </div>
        <el-form-item class="productImg" label="产品图片" prop="thumbs"
          :rules="[{ required: true, message: '请输入产品图片', trigger: 'blur' }]">
          <el-upload action="http://article.5i71.org/Communal/upload/upload_file" ref="upload" :http-request="upload" accept='image/jpeg,image/png' list-type="picture-card" :limit="3"
            :file-list="form.thumbs" :auto-upload=false :on-change="consoleFL" :on-success="handlePictureCardPreview"
        ...
        <el-form-item label="产品详情" prop="content" :rules="[{ required: true, message: '请输入产品详情', trigger: 'blur' }]">
          <div id="quillEditorQiniu">
            ...
            <el-upload class="avatar-uploader" :action="uploadUrl" accept='image/jpeg,image/png' :data="qiniuForm"
              :show-file-list="false" :on-success="uploadEditorSuccess" :on-error="uploadEditorError"
              :before-upload="beforeEditorUpload"></el-upload>
  ...
  export default {
    ...
    methods: {
      upload(data) {
        ...
        this.axios.post('http://article.5i71.org/Communal/upload/upload_file',formdata).then(res => {
      ...
      onSubmit(isrecord) {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            that.axios
              .post(
                "/MobileJson/CompanyAdmin/addproduct",
      ...
      Preservation(isrecord) {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            that.axios
              .post(
                "/MobileJson/CompanyAdmin/draftsedit",
                that.qs.stringify(that.form)
              )
              .then(function (response) {
                if (response.data.code) {
      ...
      getProcatelist() {
        ...
        that.axios
          .post(
            "/MobileJson/CompanyAdmin/procatelist",
            that.qs.stringify({
              uid: that.user.id
            })
          )
    ...
    mounted() {
      let that = this;
      that.axios
        .post("/MobileJson/Api/getQiniutoken", that.qs.stringify({}))
        .then(function (response) {
